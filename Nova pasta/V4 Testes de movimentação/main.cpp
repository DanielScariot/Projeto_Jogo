...
void update_horda(Monstro monstro[], int n_monstros, Moviment xmo[], Moviment ymo[]);
...
int main(int argc, char const *argv[]) {
    ...
    Moviment xmo[n_monstros];
    Moviment ymo[n_monstros];
    ...
    while (!GameOver)
    {
        ...
        if(evento.type == ALLEGRO_EVENT_TIMER){ //Evento de renderiza�ao
            ...
            update_horda(monstro, n_mostros, xmo, ymo);
            UpdateTiro(tiro);
...
void update_horda(Monstro monstro[], int n_monstros, Moviment xmo[], Moviment ymo[]){
    for(int n = 0; n < n_monstros; n++){
        float xmo[n];
        float ymo[n];
        int xn = monstro[n].xlocation/l_celula;
        int yn = monstro[n].ylocation/a_celula;
