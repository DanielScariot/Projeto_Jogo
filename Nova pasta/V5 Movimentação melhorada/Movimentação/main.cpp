        ...
        if(!monstro[m].stillalive)
        {
            ...
            for (j = 0; j < A; j++)
            {
                for(i = 0; i < B; i++ )
                {
                    if (mapa[i][j] == 6)
                    {
                        monstro[m].xlocation = 100 -((m - 1) *34);
                        monstro[m].ylocation = i*a_celula;
                        printf ("\nmonstro %d esta vivo = %d\n", m, monstro[m].stillalive);
                    }
                }
            }
        }
...
void update_horda(Monstro monstro[], int mapa[A][B], int n_monstros)
{
    ...
    for(m = 0; m < n_monstros; m++)
    {
        int xn = (monstro[m].xlocation + 28)/l_celula;
        int yn = (monstro[m].ylocation + 22)/a_celula;
        int xm = monstro[m].xlocation/l_celula;
        int ym = monstro[m].ylocation/a_celula;
        if(mapa[ym][xm] == 6)
        {
            monstro[m].movimentoy = 0;
            monstro[m].movimentox = 0.5;
            printf("\nmovimento x 6 = %f\n", monstro[m].movimentox);
        }
        if(mapa[ym][xm] == 0)
        {
            monstro[m].movimentoy = monstro[m].movimentoy;
            monstro[m].movimentox = monstro[m].movimentox;
        }
        if(monstro[m].movimentox == -0.5)
        {
            if(mapa[yn][xn] == 1) //ir para baixo
            {
                monstro[m].movimentoy = 0.5;
                monstro[m].movimentox = 0;
            }
        }
        if(monstro[m].movimentox != -0.5)
        {
        ...
        if(mapa[ym][xm] == 2) //ir para cima
        {
            monstro[m].movimentoy = -0.5;
            monstro[m].movimentox = 0;
        }
        if(mapa[ym][xm] == 3) // ir para esquerda
        {
            monstro[m].movimentoy = 0;
            monstro[m].movimentox = -0.5;
        }
        if (monstro[m].movimentoy == -0.5 && monstro[m].movimentox == 0)
        {
            if(mapa[yn][xn] == 4) // ir para direita
            {
                monstro[m].movimentoy = 0;
                monstro[m].movimentox = 0.5;
            }
        }
        if (monstro[m].movimentoy > 0)
        {
            if(mapa[ym][xm] == 4)
            {
                monstro[m].movimentoy = 0;
        ...
        if(monstro[m].ylocation >= ALTURA_TELA)
        {
            monstro[m].stillalive = false;
        }
        monstro[m].xlocation += monstro[m].movimentox*monstro[m].speed;
        monstro[m].ylocation += monstro[m].movimentoy*monstro[m].speed;
        printf ("\nmonstro %d esta vivo = %d\n", m, monstro[m].stillalive);
    }
